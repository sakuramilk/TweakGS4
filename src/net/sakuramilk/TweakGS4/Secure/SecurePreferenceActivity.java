...
package net.sakuramilk.TweakGS4.Secure;
...
public class SecurePreferenceActivity extends PreferenceActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mSetting = new SecureSetting(this);

        mDockEmulate = (ListPreference)findPreference(SecureSetting.KEY_DOCK_EMULATE);
        mDockManual = (PreferenceScreen)findPreference(SecureSetting.KEY_DOCK_MANUAL);
    ...
    private void updateDockEmulate(String value) {
        if (SecureSetting.DOCK_EMU_HW_INDEX.equals(value)) {
            mSetting.setDockEmulate(SecureSetting.DOCK_EMU_HW_VALUE);
            mDockEmulate.setSummary(Misc.getCurrentValueText(this, R.string.dock_emu_hw));
        } else if (SecureSetting.DOCK_EMU_SW_INDEX.equals(value)) {
            mSetting.setDockEmulate(SecureSetting.DOCK_EMU_SW_VALUE);
            mDockEmulate.setSummary(Misc.getCurrentValueText(this, R.string.dock_emu_sw));
        } else {
            mSetting.setDockEmulate(SecureSetting.DOCK_EMU_DISABLE_VALUE);
